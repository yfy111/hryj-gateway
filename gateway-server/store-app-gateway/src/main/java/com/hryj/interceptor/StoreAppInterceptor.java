...
/**
 * @author 李道云
 * @className: StoreAppInterceptor
 * @description: 门店端APP请求拦截器
 * @create 2018/7/12 20:27
 **/
@Slf4j
public class StoreAppInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        log.info("门店端，请求request信息:" + JSON.toJSONString(logMap));
        String referer = request.getHeader("referer");
        //直接放过的请求
        if(request_uri.startsWith("/notify") || request_uri.contains("swagger")
                || (StrUtil.isNotEmpty(referer) && referer.contains("swagger"))){
            return true;
        }
        //校验签名
        ...
        if("temporary".equals(app_channel)){
            ...
            if(StrUtil.isNotEmpty(login_token)){
                LoginCache.clearStaffAppLoginVO(login_token);
            }
        }
        //不需要登录权限的uri资源
        ...
        if(StrUtil.isNotEmpty(login_token)){
            ...
            if(staffAppLoginVO !=null){
                //刷新token过期时间
                LoginCache.expireStaffAppLoginTime(login_token);
            }else{
